...
本版本是面向 Obsidian 社区插件目录审核的修复版，集中清掉 1.1.3 提报里的 `Error` 与 `Warning` 项，让插件市场列表能稳定通过复扫。

### ✨ 用户可感知的变化

- **🟢 通知颜色更可靠**：`showError` / `showSuccess` 改用 Obsidian 推荐的 `Notice.messageEl` + CSS 类机制，颜色与字重由主题驱动，不再受静态样式赋值限制

### 🔧 优化改进

- **🧹 类型声明收紧**：Web API 列表映射、订阅知识库查表、`payload.exp` 解析等路径全部走 `Record<string, unknown>` + 类型守卫，去掉冗余 `as` 断言
- **🪜 抽象抽取**：把 `findTopicByAlias` 独立出来，消除两处重复的 `(t: unknown) as Record` 模式
- **🎯 异步事件统一**：`openTopic` / `loadTopicPage` 等 Preact `onClick` 处理器显式 `void` 化，编译期就能发现丢弃的 Promise

### 🐛 问题修复

- **🔤 资源路径正则控制字符**：把字面量 `\x00-\x1f` 换成 `\u0000-\u001f` 转义，消掉 Obsidian 目录 linter 的 "control character in regex" 告警
- **🧪 Notice mock 补 `messageEl`**：单测里 `Notice` 增加 `addClass` 桩，新通知样式路径也能在 Vitest 里跑

### 📋 上一版本亮点（1.1.3）

如果你刚装上 1.2.0，这部分是 1.1.3 一起带进来的、用户能直接感知的真功能：

- **📚 知识库同步补完**：手动按知识库同步全链路打通，OpenAPI 通道知识库同步也修了重复抓取
- **🏷️ 标签感知的手动同步过滤**：按笔记同步的挑选器现在能按标签筛，不再只匹配标题
- **🩹 标签自动修复**：迁移时自动清洗 frontmatter 中 Obsidian 不认的非法字符
- **🧪 测试覆盖大幅扩展**：新增 600+ 行测试，覆盖知识库同步、标签清洗、标签感知挑选器等路径
