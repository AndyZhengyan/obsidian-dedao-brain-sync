...
export function renderSourceBody(value: string): string {
  const body = normalizeSourceBody(value);
  if (body.includes(SOURCE_BODY_START) || body.includes(SOURCE_BODY_END)) {
    throw new Error('Source body contains a reserved marker');
  }
  return `${SOURCE_BODY_START}\n${body}\n${SOURCE_BODY_END}`;
}

export function parseSourceBody(value: string): SourceBodyParseResult {
  const lines = normalizeSourceBody(value).split('\n');
  const starts = lines.reduce<number[]>((found, line, index) => {
    if (line === SOURCE_BODY_START) found.push(index);
    return found;
  }, []);
  const ends = lines.reduce<number[]>((found, line, index) => {
    if (line === SOURCE_BODY_END) found.push(index);
    return found;
  }, []);
  ...
  return { kind: 'valid', body: lines.slice(starts[0] + 1, ends[0]).join('\n') };
}
...
export function createSourceHash(title: string, tags: string[], body: string): string {
  const canonical = JSON.stringify({ title, tags: canonicalTags(tags), body: normalizeSourceBody(body) });
