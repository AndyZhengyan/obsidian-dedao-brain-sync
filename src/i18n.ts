export const translations: Record<string, Record<string, string>> = {
  zh: {
    ...
    'settings.connectionSuccessWithExpiry': '连接成功 · 约 {minutes} 分钟后过期',
    ...
    'settings.credentials.webTip': '临时鉴权使用网页版登录后的 Authorization Bearer Token（可含 Bearer 前缀），可选填写 xi-csrf-token。',
    'settings.authMode.openapi': 'OpenAPI鉴权（会员）',
    'settings.authMode.web': '临时鉴权（非会员）',
  ...
  en: {
    ...
    'settings.credentials.webTip': 'Temp Auth uses the Authorization Bearer token from an active GetNote web session (Bearer prefix accepted). xi-csrf-token is optional.',
