...
- **🧩 选择弹窗时间展示统一**：按笔记、知识库等选择弹窗共用同一套相对时间格式，空时间不再显示异常内容。
- **🎨 界面样式兼容性增强**：收紧复选框和侧栏按钮的 CSS 选择器，在保持现有外观的同时减少对 Obsidian 主题样式的干扰。
- **🧹 代码与样式清理**：清理解析器、设置页和搜索视图里的静态检查告警，并减少不必要的样式覆盖，保持现有行为不变。
- **⚙️ 运行时与构建链更新**：更新 Preact、TypeScript、ESLint 等兼容版本，并扩充同步边界测试，提升后续维护稳定性。

1.4.1 的核心改进仍在生效（干净的笔记卡片预览、知识库下拉刷新、标签白名单归一化和设置页定时器清理等）。
