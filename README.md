...
- **🧹 笔记卡片预览更干净**：搜索、知识库、标签选择等所有卡片预览统一剥离 Markdown 语法，看到的是正文而不是 `**加粗**`、`# 标题`、`[链接](...)` 之类的源码。
- **🗂 知识库下拉跟随鉴权模式**：切换鉴权账号或重新登录后，知识库下拉会立即刷新到最新列表，不会再用旧的鉴权上下文拉数据。
- **🏷️ 标签白名单归一化**：标签聚合器现在统一处理大小写、空格和分隔符，避免同一标签被当成多个分组。
- **🧼 设置页定时器清理**：OAuth 和设置页里的反馈定时器在页面卸载时会一并清理，避免离开页面后还在弹通知。

1.4.0 的核心特性仍在生效（OpenAPI 搜索侧边栏、模板文件、链接笔记保留原文、保守下载策略、重建本地缺失笔记等）。
