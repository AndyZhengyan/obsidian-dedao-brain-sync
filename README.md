...
- **🗂️ 按创建日期整理笔记**：可将同步笔记按创建日期组织到目录中，并提供受控的旧目录迁移流程。
- **⚠️ 部分失败不再伪装成功**：逐篇同步失败时会显示“部分失败”、错误数量与明细；旧的“成功但有失败项”日志也会自动校正。
- **🧭 同步入口更顺手**：左侧栏“同步得到大脑”面板新增完整设置入口，同时修复弹出窗口选择器和运行时设置状态问题。
- **📚 知识库同步更完整**：订阅博主内容现在会纳入知识库同步，凭证变化后标签缓存也会正确刷新。
- **🛡️ 文件写入更安全**：加强同步写入、下载路径和文件类型校验，降低异常路径或错误对象影响本地仓库的风险。

1.4.2 及更早版本的核心改进仍在生效（统一的时间展示、主题兼容、同步边界测试和构建链更新等）。
