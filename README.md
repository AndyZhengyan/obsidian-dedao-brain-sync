...
- **🔍 OpenAPI 搜索侧边栏**：在 Obsidian 侧边栏直接搜索得到大脑笔记，命中后一键「打开本地笔记」或「同步到本地」；同步状态按条隔离，互不干扰。
- **📝 新笔记模板支持**：在设置里指定一个 vault 内的 Markdown 模板，所有新同步的笔记都按模板渲染。支持 `{{title}}` 与 `{{content}}` 占位符，以及自定义 YAML 头（如固定 `tags` / `source` 字段），让得到大脑的笔记直接套进你的本地笔记规范。
- **🔗 链接笔记保留原文**：同步链接类笔记时同时保存原始网页正文，离线也能继续阅读。
- **🛡️ 保守下载策略**：本地已存在的同一篇笔记默认跳过不覆盖；需要重拉时删除本地文件后重新同步即可。
- **♻️ 重建本地缺失笔记**：远端仍存在但本地文件被误删时，插件会自动重新创建。
- **✨ 设置页与同步弹窗打磨**：知识库同步弹窗重构、附件主开关修复、标签管理增强、定时同步配置聚合等 1.3.1 系列优化一并发布。
