...
- **🗂️ 卡片式笔记选择器**：更清晰地展示标题、摘要、正文预览、标签、类型和更新时间。
- **🏷️ 标签白名单同步**：手动同步、定时同步和知识库同步均可按标签精准过滤。
- **📚 定时同步范围增强**：可指定需要自动同步的知识库和笔记类型。
- **📎 附件分类控制**：可分别开关图片、音频、视频和文档导入。
- **🔄 同步管理优化**：支持重置同步断点，并在同步历史中显示类型和标签过滤条件。
- **🛠️ 稳定性修复**：完善知识库、附加笔记和标签处理逻辑。
