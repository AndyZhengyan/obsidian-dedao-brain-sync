...
迁移可幂等重跑：“重新整理现有文件”会按当前设置再次检查，已经位于目标路径的文件保持不变。任何目标冲突、无效元数据、缺失或共享附件都会按笔记整体跳过，不生成重名副本、不覆盖已有文件；迁移失败时会尝试回滚该笔记已经完成的移动。远端同步只按当前设置为新笔记选择路径，不负责迁移历史文件，也不会改变现有的本地覆盖/跳过约定。
