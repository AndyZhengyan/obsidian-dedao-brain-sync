...
- **🛠️ 设置页折叠更紧凑**：「启用定时同步」「下载附件」主开关后面内联「更多/收起」超链接，紧凑不占位。
- **🏷️ 标签下拉自动种草**：首次打开设置页时自动从首屏 20 条笔记提取标签；搜索框输入未匹配项可即时新增。
- **📜 滚动条去重**：标签下拉去除嵌套双层滚动条。
- **🖱️ 附件主开关单次生效**：将主开关从命令式 useEffect + ToggleComponent 改为声明式 Toggle，消除"点两次才生效"的竞态。
