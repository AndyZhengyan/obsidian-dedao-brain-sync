...
- **🟢 连接状态一眼可见**：设置页用灰、绿、红状态灯区分未验证、连接正常和连接异常；自动同步失败时同时保留“上次同步失败”提示。
- **⏱️ 自动同步成为主要路径**：首次配置会引导启用定时自动同步，并以最近一次自动同步结果判断连接健康状态。
- **📎 附件控制更准确**：修复附件全开 / 全关；音频文件与音频口水稿可以独立控制，关闭音频下载时仍可保留转写文本。
- **🛡️ 同步更稳妥**：补齐全部订阅知识的分页同步，并将路径冲突等异常明确记录为失败，避免静默遗漏。

这是一次稳定性与设置体验更新。现有配置和本地数据无需迁移；升级后建议打开设置页确认连接状态灯及自动同步状态。
...
- 🔄 **真正双向同步**：得到大脑 → Obsidian 持续同步；Obsidian → 得到大脑支持手动选择 Markdown 创建笔记。
...
| 🕒 定时同步 | 按指定间隔自动同步，可限定类型、标签和知识库 |
| 🚀 启动时同步 | 定时同步启用后，可在 Obsidian 启动时自动执行一次 |
| ⬆️ 本地上传 | 手动选择本地 Markdown 创建到得到大脑 |
...
开启定时同步后，插件会按设定间隔从得到大脑同步到 Obsidian，也可设置启动时同步。

定时同步只负责下载远端变化，不会自动上传本地笔记。
